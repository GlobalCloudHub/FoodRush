<#
.SYNOPSIS
  FoodRush Enterprise Landing Zone: Internal VNet, Private DB, Internal ACA, and Windows Jumpbox VM.
#>
...
# Global Credentials (Use for DB and VM Login)
$AdminUser     = "azureuser"
...
# Subnet 2: Container Apps (Requires /23 minimum)
...
Write-Host "`n💻 Provisioning Windows Server Jumpbox VM (Takes ~3 mins)..." -ForegroundColor Yellow
...
$DbUrl = "postgresql://${AdminUser}:${AdminPass}@${PgServer}.postgres.database.azure.com:5432/${PgDbName}?sslmode=require"
...
# Deploy INTERNAL ONLY ACA Environment
...
$GatewayUrl = Deploy-App "api-gateway" "./services/api-gateway" 8080 $GatewayEnv $GatewaySecrets "internal" # Gateway is now internal too!
...
Write-Host "🔑 VM Username            : $AdminUser"
Write-Host "🔑 VM Password            : $AdminPass"
Write-Host "----------------------------------------------------------"
Write-Host "🔒 INTERNAL Frontend UI   : https://$FrontendFqdn"
Write-Host "🐘 INTERNAL Database URL  : $DbUrl"
Write-Host "=========================================================="
Write-Host "⚠️ NOTE: The Frontend and Database are completely invisible to the internet."
Write-Host "You MUST use Remote Desktop (RDP) to connect to the Jumpbox IP ($VmPublicIp) to view the website or access the database!"